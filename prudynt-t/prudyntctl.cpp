...
static void decode_quote(char *str) {
    int i = 0;
    while (str[i] != '\0') {
        if (str[i] == '%' && str[i+1] == '2' && str[i+2] == '2') {
            str[i] = '"';
            // Shift remaining characters back by 2 positions
...
static int cmd_json(int argc, char **argv) {
    ...
    char *in_str = payload.data();
    decode_quote(in_str);
    payload = in_str;

fprintf(stderr,"json payload: [%s]",payload.c_str());
...
fprintf(stderr,"json resp: [%s]",resp.c_str());
    return 0;
}
